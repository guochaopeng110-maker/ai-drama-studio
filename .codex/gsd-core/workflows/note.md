...
- **Global scope**: `F:/Projects/DramaStudio/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/Projects/DramaStudio/.codex/notes/`)
...
2. Glob `F:/Projects/DramaStudio/.codex/notes/*.md` (if directory exists) — global notes
...
Global (F:/Projects/DramaStudio/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/Projects/DramaStudio/.codex/notes/` — works in any directory
